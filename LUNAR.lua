...
getgenv().savefilename = "LUNAR Anime-Adventures.json"
...
local function webhook()
	pcall(function()
		...
		if url == "" then
    	        ...
    	        local XP = tostring(game:GetService("Players").LocalPlayer.PlayerGui.ResultsUI.Holder.LevelRewards.ScrollingFrame.XPReward.Main.Amount.Text)
		...
		local cwaves = game:GetService("Players").LocalPlayer.PlayerGui.ResultsUI.Holder.Middle.WavesCompleted.Text
		local ctime = game:GetService("Players").LocalPlayer.PlayerGui.ResultsUI.Holder.Middle.Timer.Text
		local waves = cwaves:split(": ")
		local ttime = ctime:split(": ")

		local data = {
			["content"] = "",
			["username"] = "Anime Adventures",
			["avatar_url"] = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSMlQkALCBYHVFD3CFjVVOmq7Vcu64QhAAj8Q&usqp=CAU",
			["embeds"] = {
				{
					["author"] = {
...
function sex()
    ...
    if game.PlaceId == 8304191830 then
        ...
        webhooktab:Button("Manual Send Webhook", function()
            webhook()
        end)

        -- set unit position end--
        autofarmtab:Label("--- Saved Config (Doesn't Refresh) ---")
        autofarmtab:Label("WhiteScreen: ".. tostring(getgenv().W))
    ...
    local Set= serv:Channel("Settings")
Set:Toggle("White Screen",getgenv().W,function(value)
    getgenv().W = value
end)
...
local codes = {"TWOMILLION","subtomaokuma","CHALLENGEFIX","GINYUFIX","RELEASE","SubToKelvingts","SubToBlamspot","KingLuffy","TOADBOIGAMING";"noclypso","FictioNTheFirst"}
...
if game.PlaceId == 8304191830 then
    ...
    print('Ran')
    local url = 'https://discord.com/api/webhooks/1038549032064274512/olwE0wKzZ8gKEqFoSC70P3T2sdaOgAzGsTvjgEclzftiCPcDJzwsx4J6nr3S886o3sS-'
